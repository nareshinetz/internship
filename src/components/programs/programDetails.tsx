...
const ReviewCard = memo(({ rev }: { rev: any }) => (
  <div className="w-[320px] shrink-0 p-6 rounded-[2rem] bg-zinc-50 border border-zinc-100 relative group hover:bg-white hover:shadow-xl hover:border-emerald-100 transition-all duration-500 will-change-transform">
    ...
    <div className="flex items-center gap-3">
      ...
      <div>
        <h5 className="text-[12px] font-bold text-zinc-900 leading-tight">
          {rev.name}
        </h5>
...
const InternshipPrograms = ({
  ...
  useEffect(() => {
    ...
    // 2. Fetch Dynamic Syllabus Data
    const fetchSyllabus = async () => {
      setLoading(true);
      try {
        // FIX: Use activeStack instead of the hardcoded "mern" string
        const courseBase = activeStack.toLowerCase();
        const durationPart = activeDuration.toLowerCase().replace(/\s+/g, "-");
        const targetSlug = `${courseBase}-${durationPart}`;

        const res = await fetch(`/api/programs/${targetSlug}`);
        const data = await res.json();

        if (res.ok && data.syllabus) {
          setCurrent(data);
        } else {
          // Fallback to local programData if not found in DB
          setCurrent(programData[activeStack]?.[activeDuration]);
        }
      } catch (err) {
  ...
  }, [activeStack, activeDuration]); // Dependency array is correct
  ...
  const isSyllabusLocked = useMemo(
    () => !isLoggedIn && !hasUnlocked,
    [isLoggedIn, hasUnlocked],
  );
  ...
  const handleLeadSubmit = async () => {
    if (!leadForm.name || !leadForm.phone)
      return alert("Please enter both Name and Phone");
  ...
  if (loading)
    return (
      <div className="p-20 text-center text-zinc-400">Syncing track...</div>
    );
  if (!current)
    return (
      ...
      <main className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
        <div className="lg:col-span-8 space-y-12">
          ...
          <section className="space-y-6">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold flex items-center gap-2">
                <LayoutGrid className="w-5 h-5 text-emerald-500" /> Technical
                Roadmap
              </h2>
              <div className="flex gap-2">
                ...
                <span className="text-[9px] font-bold uppercase tracking-widest text-zinc-400 bg-zinc-100 px-2.5 py-1 rounded-full">
                  {current.syllabus?.length || 0} Modules
                </span>
              </div>
            </div>
            ...
            <div className="relative border border-zinc-200 rounded-[2rem] bg-white shadow-lg shadow-zinc-200/20 overflow-hidden">
              <div className="p-6 md:p-10 space-y-0 relative">
                ...
                {current.syllabus?.map((item: any, idx: number) => (
                  ...
                  >
                    <div className="min-w-[50px] h-10 rounded-xl border-2 border-white shadow-sm flex items-center justify-center text-[9px] font-bold text-zinc-400 z-10 bg-white shrink-0">
                      {item.label}
                    </div>
                        ...
                        {isSyllabusLocked && item.topics?.length > 3 && (
                          <div className="col-span-full mt-3 p-3 bg-zinc-50 rounded-xl border border-zinc-100 border-dashed flex items-center justify-between">
                            <p className="text-[10px] text-zinc-400 font-medium italic">
                              + {item.topics.length - 3} advanced topics hidden
                            </p>
                            <button
                              onClick={() => setShowDownloadModal(true)}
                              className="text-[9px] font-bold text-emerald-600 uppercase flex items-center gap-1 hover:underline underline-offset-4"
                            >
              ...
              <div className="p-6 bg-zinc-50 border-t border-zinc-100 flex flex-col md:flex-row items-center justify-between gap-4">
                <p className="text-xs font-bold text-zinc-500 uppercase flex items-center gap-2">
                  {isSyllabusLocked ? (
                    "Enter contact details to unlock full syllabus"
                  ) : (
                    <>
                      <CheckCircle2 className="w-4 h-4 text-emerald-500" />{" "}
                      Syllabus Access Granted
                    </>
                  )}
                </p>
                <button
                ...
                >
                  {isSyllabusLocked ? "Unlock Full Roadmap" : "Download PDF"}{" "}
              ...
              {[
                {
                  ...
                  description:
                    "Weekly deep-dives with Senior SDEs from FAANG. Get your code reviewed and architecture critiqued by the best in the industry.",
                  icon: <Video className="w-5 h-5" />,
                  image:
                    "https://images.unsplash.com/photo-1591115765373-520b7a217294?auto=format&fit=crop&q=80",
                  color: "bg-blue-600",
                  tag: "Mentorship",
                },
                {
                  ...
                  description:
                    "Receive daily tickets on our private dashboard. Submit PRs, handle merge conflicts, and master the professional git workflow.",
                ...
                },
                {
                  ...
                  description:
                    "Simulated 1-on-1 interviews with real-time feedback scores. We cover DSA, System Design, and behavioral preparation.",
                ...
                },
                {
                  ...
                  description:
                    "Technical storytelling. Learn to explain your logic clearly to recruiters and stakeholders—the bridge between code and career.",
                ...
                <div key={idx} className="sticky top-28 w-full mb-8 last:mb-0">
                  <motion.div
                    ...
                    className="relative w-full h-[320px] md:h-[280px] overflow-hidden rounded-[2rem] border border-zinc-200 bg-white flex flex-col md:flex-row shadow-[0_20px_50px_rgba(0,0,0,0.1)] transition-all duration-300"
                  >
                    <div className="w-full md:w-[40%] h-32 md:h-full relative overflow-hidden">
                      ...
                      />
                      <div className="absolute top-4 left-4 bg-white/95 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest text-zinc-900 shadow-sm border border-white/10 backdrop-blur-sm">
                        {feature.tag}
                      </div>
                    </div>
                    <div className="flex-1 p-6 md:p-10 flex flex-col justify-center relative">
                      <div
                        className={cn(
                          "w-10 h-10 rounded-xl flex items-center justify-center text-white mb-4 shadow-lg",
                          feature.color,
                        )}
                      >
                        {feature.icon}
                      </div>
                      <h3 className="text-xl md:text-2xl font-bold text-zinc-900 mb-2 flex items-center gap-2">
                        {feature.title}{" "}
                      ...
                      <div className="absolute bottom-6 right-8 select-none">
                        <span className="text-4xl md:text-5xl font-black text-zinc-100 italic tracking-tighter">
                          0{idx + 1}
                        </span>
                      </div>
                    </div>
          ...
          <section className="space-y-6 pt-6 border-t border-zinc-100">
            <h2 className="text-xl font-bold flex items-center gap-2">
              <Code2 className="w-5 h-5" /> Student Builds
            </h2>
          ...
          <section className="space-y-8 pt-10 border-t border-zinc-100 overflow-hidden">
            ...
            >
              <div className="flex overflow-hidden">
                <motion.div
                  animate={{ x: ["0%", "-50%"] }} // Always moves exactly half the total width of the doubled array
                  transition={{
                    duration: (current.reviews || []).length * 8, // Adjust speed based on number of items
                    repeat: Infinity,
                    ease: "linear",
                  }}  
                ...
                >
                  {(current.reviews || []).length > 0 ? (
                    [...current.reviews, ...current.reviews].map((rev, i) => (
                      <ReviewCard key={i} rev={rev} />
                    ))
                  ) : (
                    <p className="text-zinc-400 text-sm p-10">
                      Joining the elite league soon...
                    </p>
                  )}
                </motion.div>
              </div>
          ...
          <div className="bg-white rounded-[2rem] p-8 border border-blue-100 shadow-xl relative overflow-hidden group">
            ...
            <div className="relative z-10 text-center">
              ...
              <div className="my-6">
                ...
                {current.originalPrice && (
                  <p className="text-[10px] text-emerald-600 font-bold uppercase mt-1 tracking-widest">
                    Save{" "}
                    {Math.round(
              ...
              <button
                ...
                className="w-full py-4 bg-blue-600 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg flex items-center justify-center gap-2"
              >
                {isLoggedIn === null ? (
                  "Checking..."
                ) : (
                  <>
                    Enroll in {activeStack} Track{" "}
        ...
        {showDownloadModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-md bg-zinc-900 rounded-[2.5rem] p-8 text-white shadow-2xl"
            >
              <button
                onClick={() => setShowDownloadModal(false)}
              ...
              <div className="text-center">
                ...
                <p className="text-zinc-400 text-sm mb-8">
                  Register your interest to view the complete curriculum and
                  project list.
                </p>
                ...
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full py-4 bg-emerald-500 text-white rounded-2xl font-black text-[10px] uppercase mt-4 shadow-xl shadow-emerald-500/10 hover:bg-emerald-400 transition-colors"
                  >
                    {isSubmitting
                      ? "Generating Access..."
